"""06 · Sleep + activity covariate-adjusted primary models (2-covariate set).

Robustness check addressing whether the cosinor associations can be
accounted for by behavioral patterns that also shape heart rate. Two
mechanistically motivated covariates:

    1. Total daily sleep duration  (mean Wave-2 `sleep_period_min`)
       — short sleep would reduce the contribution of low nighttime
         heart rate to the 24-hour mean
    2. Mean daily moderate-to-vigorous physical activity (`mvpa_min`)
       — more sedentary adolescents would show chronically elevated heart
         rate

Two predictor families, scale-matched on covariates:

  (1) Between-person rhythm × MEAN behavioral covariates (Wave-2 averages):
      Predictors: typical-day mesor, amplitude, acrophase (BLUPs)
      Covariates: mean sleep_period_min, mean mvpa_min
      (from per_wave_summary.parquet)

  (2) Within-person rhythm × SD-OF-DAILY behavioral covariates (matched
      scale): adjusting a within-person rhythm SD for a between-person
      mean would be a category error — variability needs variability as
      its scale-matched control.
      Predictors: SD daily mesor, SD daily amplitude, SD daily acrophase
      Covariates: SD across days of sleep_period_min and daily MVPA
                  (= fairly_active_min + very_active_min), computed from
                  Box daily-level CSVs at Wave 2, ≥7 valid days.

Base and adjusted models are fit on identical row sets (within each
family) so any change reflects the covariates rather than missingness.
...
def _load_sd_covariates() -> pd.DataFrame:
    """Compute per-participant Wave-2 SD-of-daily sleep duration and MVPA
    (= fairly_active + very_active min) from the Box daily-level CSVs."""
...
def main() -> None:
    ...
    for fname, label in FRAMES:
        ...
        sub_btw = df.dropna(subset=MEAN_COV).copy()
        ...
        for col, plabel in BETWEEN:
            ...
            adj = fit_logistic_cluster(sub_btw, [col, *MEAN_COV],
