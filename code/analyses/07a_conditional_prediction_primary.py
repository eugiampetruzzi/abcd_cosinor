"""07a · Conditional prediction — Design B (PRIMARY).

Among adolescents whose anchor condition was established by the time of the
Wave-2 cosinor measurement (positive at W1 OR W2) and who were target-
negative at Wave 2, does each Wave-2 cosinor feature predict subsequent
onset of the target condition at Wave 3 or Wave 4?

At-risk cohort (per anchor × target):
    - anchor_first_wave in {W1, W2}              (anchor established by W2)
    - target observed and == 0 at W2
    - target observed at least once at W3 or W4  (follow-up exists)

Outcome: target_first_wave in {W3, W4}.
Predictors: cosinor_mesor_w2, cosinor_amplitude_w2, cosinor_acrophase_w2.
Run separate single-predictor logistic models (one per cosinor feature).
Per-SD standardization within each at-risk cohort.
Inference: family-clustered SEs (via fit_logistic_cluster).

Outputs:
    results/tables/conditional_prediction_primary.tsv
    results/outputs/07a_conditional_prediction_primary.log
"""
...
OUT_PATH = TABLES_DIR / "conditional_prediction_primary.tsv"
LOG_PATH = OUTPUTS_DIR / "07a_conditional_prediction_primary.log"
...
def at_risk_design_B(df: pd.DataFrame, anchor_wave_col: str,
                      target_at_w2: str, target_first_wave: str,
                      target_obs_w3w4: str) -> pd.DataFrame:
    return df[(df[anchor_wave_col].isin([W1, W2]))
               & (df[target_at_w2] == 0)
...
def main() -> None:
    ...
    print("DESIGN B (PRIMARY): anchor positive at W1 OR W2, target negative at W2,")
    ...
    for anchor_short, anchor_label, anchor_wave_col in ANCHORS:
        for tgt_short, tgt_label, tgt_first, tgt_w2, tgt_obs in TARGETS:
            if anchor_short == tgt_short:
                continue
            sub = at_risk_design_B(df, anchor_wave_col, tgt_w2, tgt_first, tgt_obs)
